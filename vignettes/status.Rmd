...
The regional summary statistics described above are also compiled into a single dataset covering all species with eBird Status Data Products, rather than being split across the individual species data packages. This dataset can be accessed with `ebirdst_regional_stats()`. Unlike most data products, which are downloaded with `ebirdst_download_status()` and then loaded with a `load_*()` function, `ebirdst_regional_stats()` downloads the file on first use, prompting for confirmation, and loads it in a single step. Subsequent calls load the already downloaded file directly.
