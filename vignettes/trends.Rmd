...
Data users who are not comfortable in R should consider downloading the data from the [eBird Status and Trends website](https://science.ebird.org/en/status-and-trends/download-data).
...
Note that some season dates span two calendar years, for example American Pipit has 2011-2021 trends estimates for a non-breeding season defined as December 20 to February 8. In this case, the first season will be December 20, 2011 to February 8, 2012.

```{r crossing}
filter(trends_runs, common_name == "American Pipit")
```

# Downloading data {#download}

Trends data access is granted through the same process as the eBird Status Data Products. If you haven't already requested an API key, consult the relevant section in the [Introduction to eBird Status Data Products vignette](articles/status.html#access).

Trends data can be downloaded one or more species using `ebirdst_download_trends()`, where the first argument is a vector of common names, scientific names, or species codes. As with the Status Data Products, trends data will be downloaded to a centralized directory which can be viewed with `ebirdst_data_dir()`. For example, let's download the breeding season trends data for Sage Thrasher.
...
Once the data are downloaded, the trends data for a set of species, can be loaded into R using the function `load_trends()`. For example, we can load the Sage Trasher trends estimates we just downloaded with:
...
trends_runs %>% 
  ...
  select(species_code, trends_season,
         trends_start_year, trends_end_year,
         trends_start_date, trends_end_date)
```
...
plot(ppy_raster[["abd_ppy"]], 
     ...
     main = "Sage Thrasher relative abundance trend\n2012-2022 [% change per year]",
     cex.main = 0.75,
...
trends_states_folds <- trends_sagthr_sf %>%
  st_drop_geometry() %>%
  group_by(state_code, state_name, fold) %>%
