...
> **Goal:** for a suite of species that breed in a focal region, identify where those species concentrate during the non-breeding season, weighted by how much of each species' breeding population the focal region holds. This information can be used to highlight "stewardship connections" between geographies, helping conservation organizations in one region identify and engage partners in the other.

Migratory species link the geographies they pass through over the course of the year, and conservation investments in one location can have consequences for species far away during other seasons. For example, protecting breeding habitat in the eastern United States has little value if the same species' non-breeding habitat in Central America is lost. [Lello-Smith et al. (2026)](https://doi.org/10.1016/j.biocon.2025.111551) demonstrate how eBird Status Data Products can be used to identify these "stewardship connections": for a suite of species that breed in a focal region, the areas where those species concentrate during the non-breeding season, weighted by how much of each species' breeding population the region holds.
...
We'll start by identifying the species that breed in West Virginia, then use the non-breeding proportion of population rasters for those species, weighted by how much of each species' breeding population occurs in West Virginia, to build a map of important non-breeding areas.

```{r connections-wv-species}
# species with at least 1% of their breeding population in west virginia,
# restricted to species with both a breeding and non-breeding season that
# passed expert review, so both seasonal layers exist
wv_breeding <- regional_stats |>
  filter(region_name == "West Virginia", season == "breeding") |>
  filter(total_pop_percent >= 0.01) |>
  inner_join(
    filter(
      ebirdst_runs,
      !is_resident,
  ...
  select(species_code, weight = total_pop_percent) |>
  arrange(desc(weight))
nrow(wv_breeding)
head(wv_breeding)
```
...
neotropics_ext <- st_bbox(
  ...
  st_as_sfc() |>
  st_transform(crs(stewardship_wv)) |>
  vect() |>
  ext()
...
v <- values(stewardship_wv, na.rm = TRUE, mat = FALSE)
v_nonzero <- v[v > 0]
cutoff <- quantile(v_nonzero, 0.5)
stewardship_wv_binned <- ifel(stewardship_wv < cutoff, 0, stewardship_wv)
...
image.plot(
  ...
  col = pal,
  smallplot = c(0.8, 0.83, 0.15, 0.85),
  legend.only = TRUE,
  axis.args = list(
    at = c(bin_width / 2, 3 * bin_width / 2, 1 - bin_width / 2),
    labels = c("Low", "Moderate", "High"),
    col.axis = "black", fg = NA, cex.axis = 0.9, lwd.ticks = 0
  )
)
```
...
peten_nonbreeding <- regional_stats |>
  ...
  select(species_code, weight = total_pop_percent) |>
  arrange(desc(weight))
nrow(peten_nonbreeding)
head(peten_nonbreeding)
```
...
for (i in seq_len(nrow(peten_nonbreeding))) {
  species_code <- peten_nonbreeding$species_code[i]
  weight <- peten_nonbreeding$weight[i]
  prop_pop <- load_raster(
    species = species_code,
    product = "proportion-population",
    period = "seasonal",
...
stewardship_peten <- sum(rast(breeding_layers), na.rm = TRUE)
```
...
na_ext <- st_bbox(c(xmin = -170, xmax = -50, ymin = 14, ymax = 75), crs = 4326) |>
  st_as_sfc() |>
  st_transform(crs(stewardship_peten)) |>
  vect() |>
  ext()
...
v <- values(stewardship_peten, na.rm = TRUE, mat = FALSE)
v_nonzero <- v[v > 0]
cutoff <- quantile(v_nonzero, 0.5)
stewardship_peten_binned <- ifel(stewardship_peten < cutoff, 0, stewardship_peten)
v_upper <- v_nonzero[v_nonzero >= cutoff]
breaks <- c(0, quantile(v_upper, seq(0, 1, by = 0.1)))
pal <- c("#e6e6e6", ebirdst_palettes(length(breaks) - 2))
...
plot(countries, col = "#cfcfcf", border = "#ffffff", add = TRUE)
plot(stewardship_peten_binned,
  breaks = breaks, col = pal,
  maxcell = ncell(stewardship_peten_binned),
  legend = FALSE, add = TRUE
)
plot(vect(countries), border = "#ffffff", lwd = 1, add = TRUE)
