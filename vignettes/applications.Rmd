...
As covered in the [Introduction to eBird Status Data Products vignette](https://ebird.github.io/ebirdst/articles/status.html), the function `load_raster()` loads raster data products into R as `SpatRaster` objects, which we can work with using the `terra` package. Let's start by loading the seasonal relative abundance raster for the example Yellow-bellied Sapsucker data. We'll work with the low resolution (27 km) data in this vignette to ensure fast processing times.
...
# Migration chronologies with uncertainty {#chron}

Next, we'll look at the temporal component of the relative abundance data. Using the weekly relative abundance cubes, we can chart the change in relative abundance throughout the year for a fixed location. Furthermore, using the upper and lower confidence interval rasters, we can add uncertainty estimates. We often refer to these as migration chronologies.
