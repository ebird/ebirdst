...
As covered in the [Introduction to eBird Status Data Products vignette](articles/status.html), the function `load_raster()` loads raster data products into R as `SpatRaster` objects, which we can work with using the `terra` package. Let's start by loading the seasonal relative abundance raster for the example Yellow-bellied Sapsucker data. We'll work with the low resolution (27 km) data in this vignette to ensure fast processing times.
